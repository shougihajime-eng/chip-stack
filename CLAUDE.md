...
- ✅ 直近で済んだこと: 販売（製品化）に向けた「データの安心」強化 — 設定ページ新設（バックアップ保存／復元／全消去・バックアップ催促）、安全＆年齢注意フッター、クラウド画面の「試験運用中」表現を製品らしく刷新、スマホ下メニュー＋PC上メニューに「設定」追加。販売用の文章（紹介文・プライバシー・年齢注意）を `docs/` に用意。ビルド・型チェックOK
- 🟡 進行中: なし
- 🔜 次の一歩: 公開（git push → Vercel 自動デプロイ）。任意で /privacy・/terms のアプリ内ページ化、月間目標機能の追加
